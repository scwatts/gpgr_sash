...
#chord_sv_df <- sigrap::chord_mantavcf2df(params$somatic_sv_vcf)
...
bpi_enabled <- gpgr::is_vcf_bpi(params$somatic_sv_vcf)
...
qc_summary_all <- dplyr::tribble(
  ...
  4, "BPI Enabled", ifelse(bpi_enabled, "TRUE", "FALSE"),
  ifelse(bpi_enabled, "", "BPI was skipped, so SV tables will not have AF_BPI/AF_PURPLE values."),
...
qc_summary_all %>%
  ...
  gt::tab_style(
    ...
    locations = cells_body(
      columns = value,
      rows = grepl("FALSE", value) & variable == "BPI Enabled"
    )
  ) %>%
  gt::tab_style(
    style = list(
...
details::details(sv_detail_table(svtab_raw), lang = "none", summary = "Column Description")
svtab_rawdt
```
...
sv_BND_main <- sv_BND %>%
  dplyr::select(
    nrow, vcfnum, `Tier (Top)`, Start, End,
    BND_ID, BND_mate,
