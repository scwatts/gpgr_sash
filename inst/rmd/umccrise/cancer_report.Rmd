...
snv_summary <- params$somatic_snv_summary %>%
  jsonlite::read_json()
snvs_max <- 500000
hypermutated <- with(snv_summary, !is.null(pregnomad) && pregnomad > snvs_max && !is.null(gnomad) && gnomad > snvs_max)
snv_summary <- snv_summary %>%
  base::unlist() %>%
  tibble::enframe() %>%
  dplyr::mutate(
    value = base::format(value, big.mark = ",", trim = TRUE),
    x = glue("{name}: {value}")
  ) %>%
  dplyr::pull(x) %>%
  base::paste(collapse = "; ")
...
if (no_sv_found) {
  ...
  sv_summary <-
    list(
      unmelted = sv_unmelted %>% dplyr::select(Type),
...
qc_summary_all <- dplyr::tribble(
  ...
  ifelse(hypermutated, "More than 500K SNVs detected, so keep only small variants within UMCCR cancer gene regions (Ensembl v107).", ""),
  ...
  21, "CNVs (Germline)", glue::glue(
    "Min: {cnv_summary[['germ']]$Min}; ",
...
details::details(knitr::kable(kat$description), lang = "none", summary = "Column Description")
if (nrow(kat$data) > 0) {
  DT::datatable(kat$data,
...
purple_plot_somaticrainfall <- file.path(img_dir, paste0(bnm, ".somatic.rainfall.png"))
if (file.exists(purple_plot_somaticrainfall)) {
  knitr::include_graphics(purple_plot_somaticrainfall)
} else {
  cat("No PURPLE rainfall plot was output (probably because there are > 100,000 SNVs)")
}
```
...
### __Copy number alterations__ {.tabset .tabset-fade #cnvs}

```{r svtab_cnv}
cnv <- gpgr::purple_cnv_som_ann_process(params$purple_som_cnv_ann)
...
cnv_main <- cnv_temp %>%
  ...
  dplyr::select(
    nrow, `Tier (Top)`, Type, Chr, Start, End, Effect,
    Genes, Transcript, Detail, CN, `CN Min+Maj`, `Start/End SegSupport`, Method,
    `BAF (count)`, `GC (windowCount)`
  )

gpgr::write_tsvjsongz(cnv_main, glue("sv/{bnm}-01_cnv_main"), result_outdir)

cnv_main_manygenes <- cnv_temp %>%
  dplyr::filter(ngen > max_genes) %>%
  dplyr::select(nrow, `Tier (Top)`, Type, Start, End,
    Effect, ngen,
    Genes = Genes_original
  )

gpgr::write_tsvjsongz(cnv_main_manygenes, glue("sv/{bnm}-02_cnv_main_manygenes"), result_outdir)

cnv_main_manytx <- cnv_temp %>%
...
gpgr::write_tsvjsongz(cnv_main_manytx, glue("sv/{bnm}-03_cnv_main_manytx"), result_outdir)
...
cnv_main_manygenesdt <- cnv_main_manygenes %>%
  DT::datatable(
    filter = list(position = "top", clear = FALSE, plain = TRUE),
    class = "cell-border display compact",
...
cnv_main_manytxdt <- cnv_main_manytx %>%
  DT::datatable(
    filter = list(position = "top", clear = FALSE, plain = TRUE),
    class = "cell-border display compact",
    rownames = FALSE, extensions = c("Buttons", "KeyTable"),
...
```{r cnv_main}
#details::details(sv_detail_table(cnv_main), lang = "none", summary = "Column Description")
cnv_maindt
```
...
```{r cnv_main_manygenes}
#details::details(sv_detail_table(cnv_main_manygenes), lang = "none", summary = "Column Description")
cnv_main_manygenesdt
```

#### Many Transcripts

```{r cnv_main_manytx}
#details::details(sv_detail_table(cnv_main_manytx), lang = "none", summary = "Column Description")
cnv_main_manytxdt
```
...
## __Structural Variants__ (unfiltered) {#sv-summary-all}
